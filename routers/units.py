# archivo: routers/units.py (VERSIÓN COMPLETA)
...
def read_units_for_project(project_id: UUID, conn = Depends(get_db)):
    db_project = crud.get_project(conn=conn, project_id=project_id)
    if db_project is None:
        raise HTTPException(status_code=404, detail="Project not found")

    units = crud.get_units_by_project(conn=conn, project_id=project_id)
    return units

@router.post("/", response_model=schemas.Unit, status_code=201)
def create_new_unit(unit: schemas.UnitCreate, conn = Depends(get_db)):
    return crud.create_unit(conn=conn, unit=unit)

@router.put("/{unit_id}", response_model=schemas.Unit)
def update_existing_unit(unit_id: UUID, unit: schemas.UnitUpdate, conn = Depends(get_db)):
...
def delete_existing_unit(unit_id: UUID, conn = Depends(get_db)):
    deleted_unit = crud.delete_unit(conn=conn, unit_id=unit_id)
    if deleted_unit is None:
        raise HTTPException(status_code=404, detail="Unit not found")
    return {"message": "Unit deleted successfully", "deleted_unit": deleted_unit}
from fastapi import APIRouter, Depends, HTTPException
from typing import List
from uuid import UUID
import schemas
